...
void OLED_WR_Byte(int fd,u8 dat,u8 mode){
if(mode){wiringPiI2CWriteReg8(fd,0x40,dat);}
else{wiringPiI2CWriteReg8(fd,0x00,dat);}
}
//SSD1306初始化
...
void OLED_ColorTurn(int fd,u8 i){
	if(i==0){
			OLED_WR_Byte(fd,0xA6,OLED_CMD);//正常显示
		}
	if(i==1){
			OLED_WR_Byte(fd,0xA7,OLED_CMD);//反色显示
		}
}
...
void OLED_DisplayTurn(int fd,u8 i){
	if(i==0){
			OLED_WR_Byte(fd,0xC8,OLED_CMD);//正常显示
			OLED_WR_Byte(fd,0xA1,OLED_CMD);
		}
	if(i==1){
			OLED_WR_Byte(fd,0xC0,OLED_CMD);//反转显示
			OLED_WR_Byte(fd,0xA0,OLED_CMD);
		}
}

//开启OLED显示 
void OLED_DisPlay_On(int fd){
	OLED_WR_Byte(fd,0x8D,OLED_CMD);//电荷泵使能
	OLED_WR_Byte(fd,0x14,OLED_CMD);//开启电荷泵
	OLED_WR_Byte(fd,0xAF,OLED_CMD);//点亮屏幕
}

//关闭OLED显示 
void OLED_DisPlay_Off(int fd){
	OLED_WR_Byte(fd,0x8D,OLED_CMD);//电荷泵使能
	OLED_WR_Byte(fd,0x10,OLED_CMD);//关闭电荷泵
	OLED_WR_Byte(fd,0xAF,OLED_CMD);//关闭屏幕
}
...
void OLED_Refresh(int fd){
	...
	for(i=0;i<8;i++)
	{
	   OLED_WR_Byte(fd,0xb0+i,OLED_CMD); //设置行起始地址
	   OLED_WR_Byte(fd,0x00,OLED_CMD);   //设置低列起始地址
	   OLED_WR_Byte(fd,0x10,OLED_CMD);   //设置高列起始地址
	   for(n=0;n<128;n++)
		 OLED_WR_Byte(fd,OLED_GRAM[n][i],OLED_DATA);
  }
}
...
{
	OLED_WR_Byte(fd,0xb0+y,OLED_CMD);//设置行起始地址
	OLED_WR_Byte(fd,((x&0xf0)>>4)|0x10,OLED_CMD);
	OLED_WR_Byte(fd,(x&0x0f)|0x01,OLED_CMD);
}
	...
	for(y=y0;y<y1;y++)
	 {
		 ...
		 for(x=x0;x<x1;x++)
		 {
			 OLED_WR_Byte(fd,BMP[j],OLED_DATA);
			 j++;
     }
	 }
}
