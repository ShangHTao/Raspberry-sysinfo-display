...
{
    char buffer[20];
    static char return_buf[6];
    FILE *fd= popen("vcgencmd measure_temp","r");
    //返回temp=53.0'C  +5取到53
    if (!fd) {  
...
char *getCPUuse(void)
{
    ...
    FILE *fd= popen("top -n1 | awk '/Cpu\\(s\\):/ {print $2}'","r");
    if (!fd) {  
        fprintf(stderr, "Erro to popen");  
    } 
    fgets(buffer, 20, fd) != NULL;
    return buffer;
}
